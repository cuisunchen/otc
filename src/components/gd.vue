<template>
  <div class="content">
    ...
    <div class="top">
      ...
      <div class="right"  @click="toggle">{{$t('Text.gdls')}}</div>
    </div>
     ...
     <div class="login-pay">
       <div class="pay-text-zone">
         <div class="pay-item"  >
             ...
             <mu-auto-complete  class="mu-text-field-input" v-model.trim="alipay_name"  :hintText="$t('Text.gdTitle')"  fullWidth    @focus="input_focus('alipay_name')"  :errorText="alipay_name_error"/> 
      </div>
        <mu-text-field  v-model.trim="alipay_account" class="mu-text-field-input" :hintText="$t('Text.gdContext')" fullWidth multi-line :rows="4" :max-length="500"   @focus="input_focus('alipay_account')"  :errorText="alipay_account_error"/>
      </div>
  ...
  <div class="btn-zone" style="margin-top:10em;">
        <mu-raised-button  :label="$t('Wallet_Detail.confirm_submit')" @click="load_trade" class="btn" fullWidth primary/>
      </div>
...
export default {
  ...
  methods: {
    load_trade: function() {
         let params = {
            "title":this.alipay_name,
	          "content":this.alipay_account
          }
    order
      .USER_feedback(params)
        .then(result => {
           this.showToast(this.$t("Text.tjcg"));
