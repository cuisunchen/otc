...
<div >
   ...
   <div class="exchange-data-zone">
    <div class="exchange-table-zone">
        ...
        <table class="exchange-table">
          <tbody>
            ...
            <tr  @click="select_sell(0)">
              ...
              <td class="td-red">{{asks.length>=1?asks[0][0].toFixed(3):0}}</td>
            ...
            <tr  @click="select_buy(0)">
              ...
              <td class="td-green" >{{bids.length>=1?bids[0][0].toFixed(3):0}}</td>
      ...
      <div class="exchange-input-zone" v-show="tab=='tab1'">
         ...
         <input  class="exchange-input-buy"  placeholder="差价%"   v-model="precentPrice"  >
 ...
 <div style="display:flex" v-show="isBuyMarket" ><mu-slider  :display-value='true' @change="onBuyMarketSliderChange" style="width:80%"></mu-slider><span style="font-size:0.7em;width:25%;text-align:right;padding-top:0.3em;">{{buyMarketChangeValue}}</span></div> -->
          ...
          <!--手续费0.15%，最小买入20FFC。-->
           ...
           <mu-raised-button v-show="!btnStart" @click="swtichStatus(true)" class="btn-buy" fullWidth primary>开始</mu-raised-button> 
           <mu-raised-button v-show="btnStart" @click="swtichStatus(false)" class="btn-buy" fullWidth primary>关闭</mu-raised-button> 
    ...
    <div>
      ...
      <div class="table-history">
        <div class="history_btn_zone">
          <button :class="{active:this.isHistory}" @click="orderedHistory()">已完成</button>
     ...
     <div class="order_item-list-content-zone">
      <div class="order_item-list-content-big" v-for="(item) in this.tradeds" :key="item.id" @click="goDetail(item)">
        <div class="order_item-list-content-big-content-zone">
        <div style="background: $white; text-align: left;padding:0px 1em;"><p style="border-bottom: 1px solid $border;
                    margin: 0px;
                    padding-top: 0.1em;
                    line-height: 2.6;
                    font-size: 0.85em;">{{$t('Order.OrderId')}}<span>{{item.disp_id}}</span><span style=" color: $lGray;
                        float: right;">{{item.status | getOrderStateType}}</span></p></div>
        ...
        line-height: 1;
        font-weight: 600;
        color: $tradeBuy;" :class="{amount_sell:!item.is_buy}">{{item.amount}}&nbsp;<span>{{item.trade_currency}}</span></div>
        ...
        line-height: 2.6em;
        font-size: 1em;
        position: relative;" :class="{online:item.is_online}">{{item.is_buy?item.seller_name :item.buyer_name | namechange}}</div>
                <div style=" flex: 1;" >
                    <h4 style="margin: 0.5em 0 0 0;
          line-height: 1.5em;
          font-size: 0.9em;
          text-align: left;
          ...
          font-weight: 600;
          color: $blue;">
                    <span style="border-radius: 0.15em;
            ...
            top: -0.2em;
            padding: 0.1em 0.3em;
            margin-left: 0.5em;">{{item.is_buy? $t('Trade.Buy.Title')+" "+item.crypto_currency : $t('Trade.Sell.Title')+" "+item.crypto_currency }}</span>
                    </h4>
                    <p style="margin-bottom: 1em;  margin-left: 8px;">{{$t('Wallet_Detail.count')}}&nbsp;&nbsp;{{item.qty}}&nbsp;{{item.crypto_currency}} 
                      <span style="float:right;">{{item.created | getformatDate }}</span>
                    </p>
                </div>
              </div>
        </div>
...
export default {
  ...
  methods: {
    ...
    loadTradedMore() {
      order
        .list(false, 0, 10, this.crypto_currency)
        .then(values => {
          this.tradeds = values.data.trades;
        })
        .catch(e => {
    ...
    getBuyTrade() {
      Promise.all([
        ...
        if (this.bids[0][0] > 0 && this.buys[0].price > 0) {
          ...
          if (this.passwordValue == "" || this.precentPrice == "") return;
          // console.log("dif==" + dif);
          this.difPrecent = dif * 100 / this.bids[0][0];
          if (this.bids[0][0] > this.buys[0].price) {
            //场外买币搬入
            if (dif * 100 / this.bids[0][0] >= this.precentPrice) {
